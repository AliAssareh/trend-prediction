...
def generate_predictions(time_frame):
    ...
    test_df = pd.DataFrame(train_preds.reshape(-1, 1), index=nvae.data_loader.x_train.index, columns=['decision'])
    val_df = pd.DataFrame(train_preds.reshape(-1, 1), index=nvae.data_loader.x_train.index, columns=['decision'])
    ...
    def _evaluate_the_mdd_and_live_cpnl(self, date, prices):
        if self.position == Position.Neutral.value:
            ...
            if self.position == Position.Long.value:
                ...
                self.pnl_df.loc[date, 'live_cpnl'] = self.cpnl[-1] + \
                                                     100 * (prices[-1] - self.position_price) / self.position_price
